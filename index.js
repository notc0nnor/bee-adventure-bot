...
await recipientInv.save();
  ...
  let senderInv = await Inventory.findOne({ userId: message.author.id });
  let recipientInv = await Inventory.findOne({ userId: recipient.id });

  if (!senderInv || senderInv.coins < amount) {
    return message.reply("You don't have enough coins.");
  }

  if (!recipientInv) {
    recipientInv = new Inventory({ userId: recipient.id });
  }
  ...
  message.reply(`You gave ${amount} 🪙 to <@${recipient.id}>`);
  ...
  inventoryLogChannel.send({
    embeds: [{
      ...
      description: [
        ...
        ``,
        `**Coins Transferred:** ${amount} 🪙`,
      ``,
      `<@${message.author.id}>: ${senderPrev} → ${senderInv.coins}`,
      `<@${recipient.id}>: ${recipientPrev} → ${recipientInv.coins}`
      ].join('\n'),
      timestamp: new Date(),
    }],
  });
...
if (command === '!give' && args[1] === 'flowers') {
  ...
  if (!senderInv || senderInv.flowers < amount) {
    return message.reply("You don't have enough flowers.");
  }
